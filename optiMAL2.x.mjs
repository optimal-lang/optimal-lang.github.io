...
Compiler.prototype.compile_body = function (ast, start) {
    if (start === ast.length - 1)
        return this.compile_ast(ast[start]);
    let result = "(";
...
Compiler.prototype.compile_ast = function (ast) {
    ...
    switch (ast[0]) {
        ...
        case "try": {
            let result = "(function(){try{return " + this.compile_ast(ast[1]) + "}catch(";
        ...
        case "<=":
        case ">=":
            return ("(" + this.compile_ast(ast[1]) + ast[0] + this.compile_ast(ast[2]) + ")");
        case "+":
        case "-":
        case "*":
        case "/": {
